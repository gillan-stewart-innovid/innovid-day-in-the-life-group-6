...
function App() {
  const campaignList = fooCampaigns.map((campaign) => (
    <li key={campaign.id}>
      ...
      {campaign.title} | start date: {dateFormatChange(campaign.startdate) ? dateFormatChange(campaign.startdate) : 'Invalid Date'} | end date:{' '}
      {dateFormatChange(campaign.enddate) ? dateFormatChange(campaign.enddate) : 'Invalid Date'} | Total Impressions {campaign.totalImpressions} | Total
